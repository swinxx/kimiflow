...
### Changed
- **R2 prose inversion complete:** the root `SKILL.md` is now a 13K always-loaded driver with phase details loaded from `phases/*.md`; expanded scaling rules live in `docs/kimiflow-scaling-knobs.md`; 55 protected invariant targets moved to phase files with phase-read verification, and release budgets now enforce a 15K root skill ceiling.
